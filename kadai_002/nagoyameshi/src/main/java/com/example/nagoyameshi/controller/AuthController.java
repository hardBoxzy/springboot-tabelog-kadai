...
//        		FieldError fieldError = new FieldError(bindingResult.getObjectName(), "email", "すでに登録済みのメールアドレスです。アカウントをアクティベートするには、こちらをクリックして認証メールを再送信してください。");
//	            bindingResult.addError(fieldError);
        		User user = userService.findByEmail(signupForm.getEmail()); 
        		String requestUrl = new String(httpServletRequest.getRequestURL());
        		System.out.println("★認証メールを送り始めます。★");
                signupEventPublisher.publishSignupEvent(user, requestUrl);
                System.out.println("★認証メールを送りました。★");
